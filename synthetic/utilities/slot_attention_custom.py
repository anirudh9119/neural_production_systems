...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, slots, num_slots = None):
        ...
        prior_mu_sigma = self.slots_rep_pr(slots.reshape((slots.shape[0] * n_s, self.dim)).detach())
        ...
        for t in range(self.iters):
            ...
            slots_mu_sigma = self.slots_rep(slots.reshape((slots.shape[0] * n_s, self.dim)))
            slots_mu_sigma = slots_mu_sigma.reshape((b, n_s, 2*self.dim))
            kl_loss += ((t+1)/self.iters) * self.get_kl_loss(prior, slots_mu_sigma)
        return slots, kl_loss

    def old_forward(self, inputs, slots, num_slots = None):

        b, n, d_ = inputs.shape
        n_s = num_slots if num_slots is not None else self.num_slots


        self.slots_mu_sigma = self.slots_rep(slots.reshape((slots.shape[0] * n_s, self.dim)))
        self.slots_mu_sigma = self.slots_mu_sigma.reshape((b, n_s, 2*self.dim))
        mu = self.slots_mu_sigma[:, :, :self.dim]
        sigma = self.slots_mu_sigma[:, :, self.dim: 2*self.dim]
        ...
        slots_mu_sigma = self.slots_rep(slots.reshape((slots.shape[0] * n_s, self.dim)))
        prior = slots_mu_sigma.reshape(-1, 2 * self.dim)
