...
class OldSharedBlockGRU(nn.Module):
...
class SharedBlockGRU(nn.Module):
    ...
    def __init__(self, ninp, nhid, k, n_templates):
        ...
        self.n_templates = n_templates
        print("input to template is ", ninp//k)
        self.templates = nn.ModuleList([nn.GRUCell(ninp//k,self.m) for _ in range(0,self.n_templates)])
        self.nhid = nhid

        self.ninp = ninp

        #self.gll_write = GroupLinearLayer(self.m,16, self.n_templates)
        self.gll_write = nn.Linear(self.m, 16)
        self.gll_read = GroupLinearLayer(self.m,16,1)
    ...
    def forward(self, input, h):
        ...
        #input = input.repeat(1,self.k,1)
        #input = input.reshape(input.shape[0]*self.k, input.shape[2])
        input = input.reshape(input.shape[0]*self.k, -1)
