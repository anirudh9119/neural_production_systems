...
import argparse
import numpy as np
from data import ArithmeticData, ArithmeticDataSpec
...
if args.generalize:
	print('TRAINING TO GENERALIZE')
	train_dataset = ArithmeticDataSpec(10000 * 20, 20)
else:
	print('TRAINING NORMAL')
	train_dataset = ArithmeticData(10000, 20)
val_dataset = ArithmeticData(2000, 20)
...
def train_epoch(epoch):
	...
	for i, data in tqdm(enumerate(train_dataloader)):
		if args.num_rules > 0:
			model.rule_network.reset_activations()
		if args.generalize:
			dig1, dig2, op, target = data[0].float().to(device), data[1].float().to(device), data[2].float().to(device), data[3].float().to(device)
			out = model(dig1, dig2, op)
